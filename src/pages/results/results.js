      ...
      <div className={styles.container}>
        ...
        <div className={styles.transcript}>
          <h2 className={styles.title}>
            {videoDetails.file_name}
            {/* The Exam that Kim Kardashian Failed.mp4 */}
          </h2>
          <div className={styles.transcriptContent}>
            {videoDetails.segments.map((segment,counter)=>{
              return(<><span className={styles.fancyNumber}>{counter+1}</span>
              <h4 className={styles.transcriptContentHeading}>{segment.title}</h4>
              <p>{segment.segment}</p></>)
            })
          }
